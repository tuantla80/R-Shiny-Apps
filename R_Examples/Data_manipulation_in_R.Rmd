...
```{r}
str_detect(x, "i")
str_detect(x, "^H")  # regex:  ^ start with 
str_detect(x, "i$")  # regex:  $ end with 
```
```{r}
str_extract(x, "ll")
```
...
str_replace(string, 'll', 'XX')
```
```{r}
string = 'Hello World! Hell yeah'
str_replace_all(string, 'll', 'XX')
```
```{r}
tmp <- str_split('this, is, a, test', ",") %>%
       unlist()
tmp  
```
```{r}
str_trim(tmp)  # remove leading and trailing empty
               # str_squish() will remove all empty 
```
```{r}
...
```
```{r}
mtcars_json <- toJSON(mtcars)  # may add: pretty=TRUE
mtcars_json
```
```{r}
# change json to R object
fromJSON(mtcars_json)
```
```{r}
all.equal(mtcars, fromJSON(mtcars_json))
```
```{r}
json_example <- '["test1", "test2"]'
json_example
```


```{r}
fromJSON(json_example)  # a vector
```
```{r}
fromJSON(json_example, simplifyVector = FALSE)  # a list
```
<!-- 9. Web scraping: Similar to beatifulsoup in Python -->
```{r}
library(rvest)
```
```{r}
